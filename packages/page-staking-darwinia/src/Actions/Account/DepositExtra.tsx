    ...
    return (
      <Modal.Content className='ui--signer-Signer-Content'>
        ...
        <InputBalance
          autoFocus
          className='medium'
          help={t('Amount to add to the currently deposit funds. This is adjusted using the bonded funds on the account.')}
          isError={!!amountError}
          label={t('additional deposit funds')}
          // maxValue={maxBalance}
        ...
        <GetPowerStyledWrapper>
          <p>{t('You will get')}: </p>
          {promiseMonth
            ? <p><span>{this.getKtonAmount()} {KTON_PROPERTIES.tokenSymbol}</span></p>
            : <p><span>0 {KTON_PROPERTIES.tokenSymbol}</span></p>
          }
        </GetPowerStyledWrapper>
      </Modal.Content>
    );
  ...
  private nextState (newState: Partial<State>): void {
    this.setState((prevState: State): State => {
      ...
      const extrinsic: any = (maxAdditional && maxAdditional.gte(ZERO))
        ? api.tx.staking.depositExtra(maxAdditional, promiseMonth)
        : null;
