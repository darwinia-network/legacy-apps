...
function Actions ({ accountChecked, allStashes, className, isInElection, isVisible, next, recentlyOnline, stakingOverview }: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={`staking--Actions ${className} ${!isVisible && 'staking--hidden'}`}>
      {isNewStakeOpen && (
        <StartStaking accountId={accountChecked}
          onClose={_toggleNewStake} />
      )}
      ...
      {foundStashes?.length
        ...
        : <div>
          ...
          <ActionNote onStart={_toggleNewStake}
            type='nominate' />
        </div>
      }
    </div>
